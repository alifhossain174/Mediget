                ...
                <div class="col">
                    <div class="breadcrumb__content text-center">
                        <h1 class="breadcrumb__content--title mb-25">
                            Edit My Nursing Service Request
                        </h1>
        ...
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-8 col-xl-5 col-md-8 col-12">
                    <form action="{{url('update/nursing/request')}}" method="post" class="request-medicine-form" enctype="multipart/form-data">
                        @csrf
                        @honeypot

                        <input type="hidden" name="request_slug" value="{{$data->slug}}">
                        <div class="form-group">
                            ...
                            <select class="form-select" name="service_id" data-toggle="select2" required>
                                ...
                                <option value="{{$service->id}}" @if($data->service_id == $service->id) selected @endif>{{$service->name}} <span> - {{$service->price}}</span></option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group" style="position: relative">
                            <label>Date & time</label>
                            <input type="text" id="service_date_time" value="{{$data->service_date_time}}" name="service_date_time" placeholder="Date & Time" required="required" class="datetimepicker" autocomplete="off" required/>
                            <i class="icofont-calendar" style="position: absolute; right: 18px; top: 41px; font-size: 18px;"></i>
                        </div>
                        <div class="form-group">
                            <label>Patient name</label>
                            <input type="text" name="patient_name" value="{{$data->patient_name}}" required />
                        </div>
                        <div class="form-group">
                            <label>Phone</label>
                            <input type="tel" name="patient_phone" value="{{$data->patient_phone}}" required />
                        </div>
                        <div class="form-group">
                            <label>Remarks</label>
